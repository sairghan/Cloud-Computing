...
public class AcessLog1
{
  
    public static void pathfinder( String filename )
    {
        SparkConf conf = new SparkConf().setMaster("yarn").setAppName("Accesslog1");
        ...
        //counts.saveAsTextFile( "hdfs:///user/root/output_2" );
    }
    ...
    public static String getPath(String line){
       ...
       // String lp = "^([\\d.]+) (\\S+) (\\S+) \\[([\\w:/]+\\s[+\\-]\\d{4})\\] \"(.+?)\" (\\d{3}) (\\d+)";
         ...
         if (matcher.matches()) {
             ...
             if(path.equals("/assets/js/lowpro.js"))
             return path;
             }
         return "";
    }
